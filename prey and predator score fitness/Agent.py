...
class Agent(Drawable):
    ...
    def __init__(self, surface, grid, name="", id=0, PLATFORM_WIDTH=2000, PLATFORM_HEIGHT=2000, font=None):
        ...
        self.energy_depletion_rate = 0.2  # Adjust as needed
        self.alive = True
        self.time_alive = 0
    ...
    def create_vision_lines(self):
        ...
        step = 270 // self.num_vision_lines

        # Iterate over the vision lines
        for i, angle in enumerate(range(-135, 135, step)):
                ...
                if dist_pred <= self.radius:
                    for i, line in enumerate(self.pred_lines):
                        if point_on_segment((self.x, self.y), line, (predator.x, predator.y), predator.mass*4):
                            self.lines_stats[i + self.num_vision_lines //2 ] = dist_pred / self.radius 
    ...
    def draw_vision_lines(self):
        ...
        for i,line in enumerate(self.food_lines):
            ...
            # self.drawText(str(i), end_point,self.font, self.FONT_COLOR)
        for i,line in enumerate(self.pred_lines):
            # Draw the vision line from the agent's position to the endpoint
            end_point = (int(line[0]), int(line[1]))
...
class Predator(Drawable):
    ...
    def move(self, rotate_left=0, rotate_right=0, dash_forward=0, stay=0):
        if not self.alive:
            return
