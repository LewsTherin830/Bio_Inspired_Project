...
class Prey :
    ...
    def test_ai(self, genome, config):
        ...
        agent = Agent(self.game.MAIN_SURFACE, id = 0, PLATFORM_HEIGHT=self.game.PLATFORM_HEIGHT, PLATFORM_WIDTH=self.game.PLATFORM_WIDTH)
    ...
    def divide_and_train(self, genome, genome_id, config):
        net = neat.nn.FeedForwardNetwork.create(genome, config)
        agent = Agent(self.game.MAIN_SURFACE, self.game.grid, id = genome_id, PLATFORM_HEIGHT=self.game.PLATFORM_HEIGHT, PLATFORM_WIDTH=self.game.PLATFORM_WIDTH)
        self.game_info = GameInfo(self.game.number_of_cells, [agent])
        self.game.painter.add(agent)
        show = False
        run = True
        ...
        while run:
            ...
            agent.update()
            self.game_info.update_agents_fitness()
            agent.collisionDetection(self.game.cells, [])
            ...
            if self.game_info.game_over() or agent.time_alive >= fitness_threshold:
